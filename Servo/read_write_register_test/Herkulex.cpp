...
// Add this to the end of Herkulex.cpp
int HerkulexClass::readRegistryRAM(int servoID, int address) {
    pSize = 0x09;               // Packet size
    pID   = servoID;            // Servo ID
    cmd   = HRAMREAD;           // Command: RAM Read
    data[0] = address;          // Address to read
    data[1] = 0x02;             // Length to read (2 bytes for PID)
    ...
    dataEx[0] = 0xFF;           // Header
    dataEx[1] = 0xFF;           // Header
    dataEx[2] = pSize;          // Size
    dataEx[3] = pID;            // ID
    ...
    readData(13);               // Read packet (Header + Body)

    // Verify response
    if (dataEx[3] != pID) return -1; // Wrong ID returned
    
    // Combine the two bytes (LSB + MSB) into an integer
    int value = ((dataEx[10] & 0xFF) << 8) | dataEx[9];
    return value;
}
